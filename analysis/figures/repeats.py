...
def main():
    ...
    for ss in range(start, end):
        ...
        gas_ids_repeat = np.transpose(
            (
                ordered_count.loc[ordered_count > 1].index,
                ordered_count.loc[ordered_count > 1].values,
                [ss] * len(ordered_count.loc[ordered_count > 1]),
            )
        )
        repeaters_all.append(gas_ids_repeat)
