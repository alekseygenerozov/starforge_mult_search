...
for ii, row in tqdm.tqdm(enumerate(bin_ids)):
    ...
    tmp_sel2a = high_df.loc[
        high_df.index.get_level_values("id").str.contains(rf"\b{bin_list[0]}\b")
    ]
    tmp_sel2b = high_df.loc[
        high_df.index.get_level_values("id").str.contains(rf"\b{bin_list[1]}\b")
    ]

    if len(tmp_sel2a) > 0:
        comps_a = tmp_sel2a.groupby("t")[
            ["tval", "mult", "mult_ids_list", "hier"]
        ].apply(lambda g: g[g["mult"] == g["mult"].max()])
...
##Fix the name here!!!
comp = dict(
    comps_a_ids=comps_a_ids,
    comps_a_times=comps_a_times,
    comps_a_ids_flat=comps_a_ids_flat,
    comp_b_ids=comps_b_ids,
    comps_b_times=comps_b_times,
    comps_b_ids_flat=comps_b_ids_flat,
)
