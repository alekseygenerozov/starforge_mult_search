...
for ii, row in tqdm.tqdm(enumerate(bin_ids)):
    ...
    min_mass_b[ii] = min(
        path_lookup[str(id1)][end_bin_time, mcol],
        path_lookup[str(id2)][end_bin_time, mcol],
    )

    tmp_df = high_df.xs(end_time, level="t")
    tmp_mult1 = lookup_star_mult_with_mass(
        tmp_df, id1, end_time, path_lookup, pre_filtered=True, contig_suff=contig_suff
    )
    tmp_mult2 = lookup_star_mult_with_mass(
        tmp_df, id2, end_time, path_lookup, pre_filtered=True, contig_suff=contig_suff
    )
...
print(
    len(min_mass_b[quasi_filter & (min_mass_b < 0.1)]) / len(min_mass_b[quasi_filter])
)
#########################################################################################################
##Tallying all the non-surviving states.
ns1 = (
    len(end_states[(end_states == "1 1") & (quasi_filter) & (min_mass_b > mthres)]) / d1
)
ns2 = (
    len(end_states[(end_states == "1 2") & (quasi_filter) & (min_mass_b > mthres)]) / d1
)
ns3 = (
    len(end_states[(end_states == "1 3") & (quasi_filter) & (min_mass_b > mthres)]) / d1
)
ns4 = (
    len(end_states[(end_states == "1 4") & (quasi_filter) & (min_mass_b > mthres)]) / d1
)
ns5 = []
ns5.append(
    len(
...
print(
    f"S S:{ns1:.4f} B S:{ns2:.4f} T S:{ns3:.4f} Q S:{ns4:.4f} M M:{np.sum(ns5):.4f} NS Tot: {ns1 + ns2 + ns3 + ns4 + np.sum(ns5):.4f}"
)
#########################################################################################################
##Tallying all the surviving states.
ss1 = (
    len(end_states[(end_states == "2 2") & (quasi_filter) & (min_mass_b > mthres)]) / d1
)
ss2 = (
    len(end_states[(end_states == "3 3") & (quasi_filter) & (min_mass_b > mthres)]) / d1
)
ss3 = (
    len(end_states[(end_states == "4 4") & (quasi_filter) & (min_mass_b > mthres)]) / d1
)
