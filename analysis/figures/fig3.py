...
bar_color = patches[0].get_edgecolor()
ax.hist(
    vangs_prim[quasi_filter & ~bfb_filter],
    bins=bins,
    histtype="step",
    color=bar_color,
    linewidth=1.5,
...
ax.annotate(f"Rel vel & sep", xy=(-0.875, 2.0), color=bar_color, va="bottom")

iso_height = 1.0 / (0.1) / len(bins)
(l1,) = ax.plot(
    [-1, 0.5, 0.75, 1],
    [iso_height, iso_height, iso_height, iso_height],
    "-.",
    label="Isotropic",
)
...
ax.annotate("Control", (6.0, 800), color="red")
