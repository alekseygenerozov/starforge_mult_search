...
from starforge_mult_search.analysis.high_multiples_analysis import make_hier, get_mult, get_pair_state, add_node_to_orbit_tab, add_node_to_orbit_tab_streamlined
...
for ii,star_id in enumerate(star_ids):
...
l2,=ax.plot(np.log10(absc), 0.9 * (absc / 0.3)**(-f2 + 1), color=fit_colors[1], linestyle="--", label=f"$dN/dm \\propto m^{{-{f2:.2f}}}$")
labelLines([l1], fontsize=16, xvals=(0.2,), ha='right', va='top', ang=-55, y_offset=-0.12)
