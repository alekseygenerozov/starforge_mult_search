...
from starforge_mult_search.analysis.figures.figure_preamble import my_data, path_lookup, lookup_dict, contig_suff, flat_suff, my_ft
...
end_state_all = []
orbs_all = []
init_table_columns = ["t", "id", "x", "y", "z", "vx", "vy", "vz", "mcol", "mtotcol", "hcol", "nonew_comps", "is_soft"]
end_table_columns = ["t", "id", "x", "y", "z", "vx", "vy", "vz", "mcol", "mtotcol", "hcol", "bdist", "sma", "ecc", "is_soft"]
for bin_input in range(len(ex_index)):
    ...
    end_encounter_time = start_encounter_time + (end_encounter_time - start_encounter_time) * 3
    ...
    if end_encounter_time >= len(path1):
        ...
        init_state_all.append(pd.DataFrame(dummy, columns=init_table_columns).reset_index(drop=True))
        dummy = np.atleast_2d(np.ones(len(end_table_columns)) * np.inf)
        end_state_all.append(pd.DataFrame(dummy, columns=end_table_columns).reset_index(drop=True))
        continue 
    ...
    end_orbit = get_orbit(path1[end_encounter_time, pxcol:pzcol+1], path2[end_encounter_time, pxcol:pzcol+1], 
              path1[end_encounter_time, vxcol:vzcol+1], path2[end_encounter_time, vxcol:vzcol+1],
              path1[end_encounter_time, mtotcol], path2[end_encounter_time, mtotcol],
              h1=path1[end_encounter_time, hcol], h2=path2[end_encounter_time, hcol]
              )
    ...
    init_state_all.append(pd.DataFrame(init_state, columns=init_table_columns).reset_index(drop=True))
    end_state_all.append(pd.DataFrame(end_state, columns=end_table_columns).reset_index(drop=True))
