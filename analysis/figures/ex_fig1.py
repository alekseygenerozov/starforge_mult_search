...
for bb, uid in enumerate(bin_ids_example):
    ...
    axs[0, bb].plot(
        tmp_info[:, LOOKUP_SNAP] * snap_interval / 1e6,
        tmp_info[:, LOOKUP_SMA] * cgs.pc / cgs.au,
        color="brown",
        linewidth=5,
        label="1&2 Bound",
        zorder=2,
    ...
    for kk in (0, 1):
        ...
        (l1,) = axs[0, bb].semilogy(
            [
                tmp_sys_idx[1, LOOKUP_SNAP] * snap_interval / 1e6,
                tmp_sys_idx[-2, LOOKUP_SNAP] * snap_interval / 1e6,
                tmp_sys_idx[-1, LOOKUP_SNAP] * snap_interval / 1e6,
            ],
            [20, 20, 20],
            color="0.5",
        ...
        axs[0, bb].semilogy(
            tmp_sys_idx[:, LOOKUP_SNAP][clean_filt] * snap_interval / 1e6,
        ...
        (tmp_line,) = axs[2, bb].semilogy(
            tmp_sys_idx[:, LOOKUP_SNAP] * snap_interval / 1e6,
            tmp_sys_idx[:, LOOKUP_MTOT],
            color=cols[kk],
            label=f"Star {kk + 1}",
            zorder=1,
        )
    ...
    t = axs[0, bb].text(
        tmp_sys_idx[:, LOOKUP_SNAP][clean_filt][12] * snap_interval / 1e6,
        25,
        "Softening",
        color="0.5",
    )
