...
def get_energies(p1_dat, g1_dat, p2_dat, g2_dat):
    ...
    ke = KE(
        np.vstack((blob_com, pos2)),
        np.append(np.sum(blob_mass), m2),
        np.vstack((blob_com_vel, vel2)),
        np.append(0, 0),
    )
...
base_post = base.replace(
    "/home/aleksey/Dropbox/projects/Hagai_projects/star_forge/", ""
)
bin_ids = np.load(base_post + aa + "/unique_bin_ids_mult.npz", allow_pickle=True)[
    "arr_0"
]
...
for ii, uid in enumerate(bin_ids):
    ...
    with h5py.File(
        base
        + f"/halo_masses/\
halo_masses_sing_npTrue_c0.5_{fst_idx}_compFalse_tf{sys.argv[3]}.hdf5",
        "r",
    ) as ff:
