...
def get_bound_snaps(sys1_info, sys2_info):
    ...
    same_sys_filt1 = np.isin(sys1_tag, sys2_tag)
    same_sys_filt2 = np.isin(sys2_tag, sys1_tag)
...
def get_quasi(bin_ids, lookup_dict, fst, snap_interval, cadence, path_lookup):
    ...
    for ii, uid in enumerate(bin_ids):
        ...
        fpm = (
            path1[path1[:, 0] == tmp_final_bound_snap][0, mcol]
            + path2[path2[:, 0] == tmp_final_bound_snap][0, mcol]
        )
        final_pair_mass_lbin[ii] = fpm
        fpm = (
            path1[path1[:, 0] == same_sys_snap[-1]][0, mcol]
            + path2[path2[:, 0] == same_sys_snap[-1]][0, mcol]
        )
        final_pair_mass_lsys[ii] = fpm

        ##Total time binaries are bound normalized to the period.
        tmp_bound_time_pers = (bound_snaps1[:, LOOKUP_SMA] * cgs.pc / cgs.au) ** 1.5 / (
            bound_snaps1[:, LOOKUP_MTOT] + bound_snaps2[:, LOOKUP_MTOT]
        ) ** 0.5
    ...
    return {
        "quasi_filter": (bound_time_norm >= 1) & (bound_time > 1),
        "final_bound_snaps": final_bound_snaps,
        "final_bound_snaps_norm": final_bound_snaps_norm,
        "bound_time": bound_time,
        "bound_time_norm": bound_time_norm,
        "init_bound_snaps": init_bound_snaps,
        "age_diff": age_diff,
        "same_sys_final_norm": same_sys_final_norm,
        "final_pair_mass_lbin": final_pair_mass_lbin,
        "final_pair_mass_lsys": final_pair_mass_lsys,
        "bound_time_pers": bound_time_pers,
    }
...
def get_energy(bin_ids, fst, lookup_dict, path_lookup):
    ...
    for ii, uid in enumerate(bin_ids):
        ...
        bin_at_fst[ii] = (
            (look1[:, 2] == look2[:, 2])
            and (look1[:, LOOKUP_SMA] == look2[:, LOOKUP_SMA])
        )[0]
        ...
        tmp_en_gas = find_multiples_new2.get_energy(
            pos1, pos2, vel1, vel2, mtot1, mtot2, h1=h1, h2=h2
        )
        tmp_en = find_multiples_new2.get_energy(
            pos1, pos2, vel1, vel2, m1, m2, h1=h1, h2=h2
        )
        ...
        vangs_prim[ii] = (
            np.dot(vel1 - vel2, pos1 - pos2)
            / np.linalg.norm(vel1 - vel2)
    ...
    return {
        "ens": ens,
        "ens_gas": ens_gas,
        "same_sys_at_fst": same_sys_at_fst,
        "bin_at_fst": bin_at_fst,
        "vangs": vangs,
        "vangs_prim": vangs_prim,
        "mfinal_primary": mfinal_primary,
        "mfinal_pair": mfinal_pair,
        "end_stars": end_stars,
    }
...
def main(params):
    ...
    np.savez(
        save_path + f"/dat_coll{analysis_suff}.npz",
        bin_ids=bin_ids,
        fst=fst,
        ens=en_data["ens"],
        ens_gas=en_data["ens_gas"],
        same_sys_at_fst=en_data["same_sys_at_fst"],
        bin_at_fst=en_data["bin_at_fst"],
        vangs=en_data["vangs"],
        vangs_prim=en_data["vangs_prim"],
